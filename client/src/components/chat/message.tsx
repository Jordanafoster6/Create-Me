  ...
  if (!isUser && message.content) {
    try {
      ...
      switch (jsonContent.type) {
        case "design":
          contentType = "design";
          parsedContent = jsonContent.message || "How does this design look?";
    ...
    >
      ...
      >
        {contentType === "design" ? (
          ...
          <div className="space-y-4">
            <p className="text-sm mb-2">{parsedContent}</p>
            {jsonContent.blueprints && jsonContent.blueprints.length > 0 && (
              <div className="grid grid-cols-2 gap-4">
                {jsonContent.blueprints.map((blueprint: any, index: number) => (
                  <ProductPreview
                    key={index}
                    imageUrl={blueprint.thumbnail_url}
                    productName={blueprint.title}
                  />
                ))}
              </div>
            )}
